...
class Level extends Controller
{
     ...
     * @return void
     */
    public function lists(AgentLevel $level_model) 
    {
        if (IS_AJAX) {
            ...
            $level_count = env('agent.level_count', 3);
     ...
     * @return void
     */
    public function add(AgentLevel $level_model)
    {
        if (IS_POST) {
            try {
     ...
     * @return void
     */
    public function edit(AgentLevel $level_model)
    {
     ...
     * @return void
     */
    public function del(AgentLevel $level_model)
    {
        if (IS_AJAX) {
            $level_id = (int)$this->params['level'] ?? 0;
            $level = $level_model->where([['level', '=', $level_id]])->find();
