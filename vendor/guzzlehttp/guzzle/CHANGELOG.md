...
* `Guzzle\AbstractHasDispatcher::dispatch()` now returns the event that was dispatched
...
* Added `Guzzle\Collection::setPath($path, $value)` to set a value into an array using a nested key
...
* `Guzzle\Exception\ExceptionCollection` now creates a more readable exception message
* Fixing BC break: Added back the MonologLogAdapter implementation rather than extending from PsrLog so that older
  Symfony users can still use the old version of Monolog.
* Fixing BC break: Added the implementation back in for `Guzzle\Http\Message\AbstractMessage::getTokenizedHeader()`.
  Now triggering an E_USER_DEPRECATED warning when used. Use `$message->getHeader()->parseParams()`.
* Several performance improvements to `Guzzle\Collection`
...
* Changed `Guzzle\Collection::__construct($data)` to no longer accepts a null value for `$data` but a
  default `array()`
...
* You can now enable E_USER_DEPRECATED warnings to see if you are using a deprecated method by setting
  `Guzzle\Version::$emitWarnings` to true.
...
* Marked `Guzzle\Collection::inject()` as deprecated.
...
* Adding wildcard support to Guzzle\Collection::getPath()
...
* Moved getPath from Guzzle\Service\Resource\Model to Guzzle\Collection
...
* Cleaning up `Guzzle\Collection::inject` method
...
* Added a StreamInterface, EntityBodyInterface, and added ftell() to Guzzle\Stream
...
* BC: Removing Guzzle\Batch\Batch::count() and replacing it with isEmpty()
...
* Guzzle\Service\Resource\ResourceIteratorApplyBatched now internally uses the Guzzle\Batch namespace
...
* Added Guzzle\Log\ArrayLogAdapter
...
* [BC] Guzzle\FromConfigInterface no longer requires any config options.
...
* [BC] Removing Guzzle\NullObject.  Use https://github.com/mtdowling/NullObject if you need it.
* [BC] Removing Guzzle\XmlElement.
...
* Moving Guzzle\Guzzle::inject to Guzzle\Collection::inject
...
* [BC] Simplifying Guzzle\Collection.  Please check to see if you are using features that are now deprecated.
